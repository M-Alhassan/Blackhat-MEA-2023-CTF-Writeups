...
4. **Retrieve the Notepad Process:** Start your way to find the Notepad process within the running processes. Use the command `strings running_processes.txt | grep Notepad` to extract the detials of Notepad processs. Note down the Process ID (PID), this represents the unique proccess number of the Notepad.exe process.
